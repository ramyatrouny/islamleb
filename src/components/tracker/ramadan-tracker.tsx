...
import { getRamadanDate } from "@/lib/date-utils";
import { sectionReveal } from "@/lib/animations";
import { useRamadanStore } from "@/lib/store";
import { useRamadanDate } from "@/hooks/use-ramadan-date";
import { RAMADAN_SPIRITUAL_GOALS } from "@/config/spiritual-goals";
...
export function RamadanTracker() {
  ...
  const currentDayGoals = useMemo(() => {
    return dailyGoals[activeDayIndex] || new Array(RAMADAN_SPIRITUAL_GOALS.length).fill(false);
  ...
  return (
    <div className="min-h-screen bg-background islamic-pattern">
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <PageHeader title="متتبع العبادات" subtitle="تتبع صيامك وعباداتك في رمضان" />
          ...
          <Card className="border-[#d4a574]/20 bg-card/80 backdrop-blur-sm overflow-hidden relative">
            <CardContent className="py-6 flex flex-col items-center gap-3">
              ...
              <p className="text-xl font-semibold text-foreground">
                سلسلة الصيام:{" "}
                <span className="text-[#d4a574]">{toArabicNumerals(streak)}</span> أيام
                متتالية
              </p>
              {streak >= 7 && (
            ...
            <CardContent>
              <div className="grid grid-cols-5 gap-3 sm:grid-cols-6">
                {Array.from({ length: RAMADAN_2026.TOTAL_DAYS }, (_, i) => {
                  ...
                  const isPast = todayIndex >= 0 ? i < todayIndex : false;
          ...
          <Card className="border-[#2d6a4f]/20 bg-card/80 backdrop-blur-sm">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="border-[#2d6a4f]/40 text-[#2d6a4f]">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                من {toArabicNumerals(RAMADAN_SPIRITUAL_GOALS.length)} هدفاً
              </p>
              <Progress
                value={(goalsCompleted / RAMADAN_SPIRITUAL_GOALS.length) * 100}
                className="h-2 mt-1 bg-[#2d6a4f]/20 [&>[data-slot=progress-indicator]]:bg-[#2d6a4f]"
              />
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {RAMADAN_SPIRITUAL_GOALS.map((goal, gi) => {
                  ...
                  return (
                    ...
                    >
                      <Button
                        ...
                        onClick={() =>
                          toggleGoal(activeDayIndex, gi, RAMADAN_SPIRITUAL_GOALS.length)
                        }
                        className={`
                      ...
                      >
                        <div
                          className={`
                            shrink-0 size-5 rounded-md border-2 flex items-center justify-center
                            transition-colors duration-200
                            ${checked ? "bg-[#2d6a4f] border-[#2d6a4f]" : "border-muted-foreground/40"}
                          `}
                        >
                          {checked && (
                            <motion.svg
              ...
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="grid grid-cols-2 gap-4 w-full max-w-md text-center">
                  <div className="rounded-lg bg-[#2d6a4f]/10 p-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">نسبة الصيام المنجز</p>
                  </div>
                  <div className="rounded-lg bg-[#d4a574]/10 p-4">
                    <p className="text-2xl font-bold text-[#d4a574]">
                      {toArabicNumerals(overallProgress)}٪
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">تقدم شهر رمضان</p>
                  </div>
                </div>
                ...
                <div className="w-full max-w-md">
                  ...
                  <div className="relative h-3 w-full rounded-full bg-muted/30 overflow-hidden">
                    <motion.div
                      ...
                      style={{
                        background: "linear-gradient(to left, #2d6a4f, #d4a574)",
                      }}
                      initial={{ width: "0%" }}
